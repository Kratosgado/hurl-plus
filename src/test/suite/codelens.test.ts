...
import { FIXTURES_PATH, openDocument, sleep } from "./helpers";
...
suite( "CodeLens Provider", () => {
  ...
  test( "Run Request CodeLens has correct command", async () => {
    ...
    assert.strictEqual( runLens!.command!.command, "hurl-plus.runEntry" );
  } );
  ...
  test( "HTTP response status lines stay inside the previous request entry", async () => {
    const uri = vscode.Uri.file( path.join( FIXTURES_PATH, "status-line.hurl" ) );
    const content = [
    ...
    try {
      const statusDoc = await vscode.workspace.openTextDocument( uri );
      ...
      assert.ok(
        ...
        `Expected only the request line to get CodeLens entries, got lines: ${lenses!.map( ( lens ) => lens.range.start.line ).join( ", " )}`
      );
    } finally {
      await vscode.commands.executeCommand( "workbench.action.closeActiveEditor" );
      try {
